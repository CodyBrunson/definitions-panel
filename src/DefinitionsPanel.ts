...
class DefinitionsPanel extends Plugin {
    ...
    private loadAllItems(): void {
        ...
        try {
            const itemDefMap =
                this.gameHooks.ItemDefinitionManager?._itemDefMap;
            if (!itemDefMap) {
    ...
    private loadAllNpcs(): void {
        ...
        try {
            const npcDefMap =
                this.gameHooks?.NpcDefinitionManager?._npcDefMap;
            if (!npcDefMap) {
        ...
        npcSpriteElements.forEach(spriteElement => {
            ...
            if (typeInfo.isCreature && typeInfo.creatureType !== undefined) {
                ...
                const spritesheetManager =
                    this.gameHooks?.SpriteSheetManager?.Instance;
    ...
    private removeSpriteReference(
        ...
        if (newCount === 0) {
            ...
            try {
                const blobLoader =
                    this.gameHooks?.BlobLoader?.Instance;
    ...
    private getSkillName(skillId: number): string {
        try {
            return (
                this.gameLookups.Skills[skillId] || `Skill ${skillId}`
            );
        } catch {
    ...
    private getEquipmentTypeName(typeId: number): string {
        try {
            return (
                this.gameLookups.EquipmentTypes[typeId] ||
                `Type ${typeId}`
            );
        } catch {
            return `Type ${typeId}`;
    ...
    private loadNpcDetailsIntoModal(
        ...
        try {
            const npcDef =
                this.gameHooks?.NpcDefinitionManager?.getDefById(
                    npcId
                );
            if (!npcDef) {
                container.innerHTML =
            ...
            if (
                ...
                npcDef._combat._spellIds.forEach((spellId: number) => {
                    try {
                        const spellDef =
                            this.gameHooks?.SpellDefinitionManager?.getDefById(
                                spellId
                            );
            ...
            //             equippedItems.forEach((item: any) => {
            //                 try {
            //                     const itemDef = this.gameHooks.ItemDefinitionManager._itemDefMap?.get(item._id);
            //                     const itemName = itemDef?._nameCapitalized || itemDef?._name || `Item ${item._id}`;
            //                     const itemPos = this.gameHooks.InventoryItemSpriteManager?.getCSSBackgroundPositionForItem(item._id);
